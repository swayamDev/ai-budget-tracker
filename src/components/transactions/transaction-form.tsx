...
export default function TransactionForm({
  ...
  const {
    ...
    // Cast required: z.coerce.number() has `unknown` input type in Zod v4,
    // which conflicts with RHF's strict Resolver<TFieldValues> generic.
          ...
          {(["INCOME", "EXPENSE"] as const).map((t) => (
            <label key={t} className="cursor-pointer">
              <input
                ...
                className="sr-only"
                aria-label={
                  t === "INCOME" ? "Income transaction" : "Expense transaction"
                }
              />
              <div
                aria-hidden="true"
