    ...
    <>
      ...
      >
        ...
        <div className="h-16 flex items-center px-4 border-b border-sidebar-border shrink-0">
          <div
            className="w-8 h-8 rounded-xl bg-primary flex items-center justify-center shrink-0 shadow-glow-sm"
            aria-hidden="true"
          >
        ...
        {/* Navigation */}
        <nav aria-label="Dashboard navigation" className="flex-1 p-2.5 space-y-0.5 overflow-y-auto scrollbar-hide">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                {!sidebarOpen && (
                  <div
                    role="tooltip"
                    className="absolute left-full ml-3 px-2.5 py-1.5 bg-popover border border-border rounded-lg text-xs font-medium text-foreground shadow-lg whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50"
                  >
                    {item.title}
                  </div>
                )}
