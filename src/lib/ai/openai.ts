...
export async function generateFinancialInsights(transactions: Array<{
  ...
  const summary = transactions
    ...
    .map(
      (t) =>
        `${t.type} $${t.amount} on ${t.category} (${new Date(t.date).toLocaleDateString()})${t.note ? ` - ${t.note}` : ''}`
    )
    .join('\n');
...
export async function predictFutureExpenses(
  transactions: Array<{ type: string; amount: number; category: string; date: Date | string }>
): Promise<string> {
